...
#
#BENNY  You need to add the base file name in web dest path
module Jekyll
  module Tags
    class Tikz < Liquid::Block
      ...
      def render(context)
        ...
        if !File.exist?(tex_path) or !tikz_same?(tex_path, tikz_code) or !File.exist?(dest_path)
          ...
          #          system("inkscape -l #{dest_path}   #{pdf_path}")
        end

        web_dest_path = File.join("/mathemattic/img", File.basename(context["page"]["url"], ".*"), "#{@file_name}.svg")
